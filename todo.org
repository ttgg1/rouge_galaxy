...
- [ ] if no encounter -> real time mode -> bool isSimulating in gamestruct
- +game struct+
...
* list.c
- [ ] on destroy function pointer, which is called on destroy on every object to free
* ui.c
- [X] open textbox
- select items -> uiInput
- look like pokemon fight menu

* uiInput.c
- [ ] open popup window, waiting for input -> capture keypresses -> spawn thread, which handles the input capture or game Event hook
